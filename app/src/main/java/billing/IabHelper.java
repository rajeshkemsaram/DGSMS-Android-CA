...
public class IabHelper {
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
        ...
        mServiceConn = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                Log.v("Helper","packageName "+packageName);
    ...
    public void launchPurchaseFlow(Activity act, String productId, int requestCode,
            OnIabPurchaseFinishedListener listener, String extraData)
        throws IabAsyncInProgressException {
        launchPurchaseFlow(act, productId, ITEM_TYPE_INAPP, null, requestCode, listener, extraData);
    }

    public void launchSubscriptionPurchaseFlow(Activity act, String productId, int requestCode,
            OnIabPurchaseFinishedListener listener) throws IabAsyncInProgressException {
        launchSubscriptionPurchaseFlow(act, productId, requestCode, listener, "");
    }

    public void launchSubscriptionPurchaseFlow(Activity act, String productId, int requestCode,
            OnIabPurchaseFinishedListener listener, String extraData)
        throws IabAsyncInProgressException {
        launchPurchaseFlow(act, productId, ITEM_TYPE_SUBS, null, requestCode, listener, extraData);
    }
     ...
     */
    public void launchPurchaseFlow(Activity act, String productId, String itemType, List<String> oldSkus,
        ...
        try {
            logDebug("Constructing buy intent for " + productId + ", item type: " + itemType);
            Bundle buyIntentBundle;
            if (oldSkus == null || oldSkus.isEmpty()) {
                // Purchasing a new item or subscription re-signup
                buyIntentBundle = mService.getBuyIntent(3, mContext.getPackageName(), productId, itemType,
                        extraData);
            } else {
                ...
                buyIntentBundle = mService.getBuyIntentToReplaceSkus(5, mContext.getPackageName(),
                        oldSkus, productId, itemType, extraData);
            }
            ...
            logError("SendIntentException while launching purchase flow for productId " + productId);
            e.printStackTrace();
            flagEndAsync();
            ...
            logError("RemoteException while launching purchase flow for productId " + productId);
            e.printStackTrace();
            flagEndAsync();
        ...
        if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
            ...
            try {
                ...
                if (!Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
                    logError("Purchase signature verification FAILED for productId " + productId);
                    result = new IabResult(IABHELPER_VERIFICATION_FAILED, "Signature verification failed for sku " + productId);
    ...
    public Inventory queryInventory(boolean querySkuDetails, List<String> moreItemSkus,
        ...
        try {
            ...
            Log.v("Suman", "mSubscriptionsSupported "+mSubscriptionsSupported);
...
//        }
        try {
            ...
            logDebug("Consuming productId: " + productId + ", token: " + token);
            int response = mService.consumePurchase(3, mContext.getPackageName(), token);
            if (response == BILLING_RESPONSE_RESULT_OK) {
                logDebug("Successfully consumed productId: " + productId);
            }
            else {
                logDebug("Error consuming consuming productId " + productId + ". " + getResponseDesc(response));
                throw new IabException(response, "Error consuming productId " + productId);
            }
        }
        catch (RemoteException e) {
